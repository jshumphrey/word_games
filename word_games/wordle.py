...
class WordleWord(word_games.Word):
    ...
    def calculate_guess_results(self, guessed_word: WordleWord) -> str:
...
def solve_wordle(
    ...
    """
    target_word = WordleWord(target_word) if isinstance(target_word, str) else target_word
    num_guesses = 0
    masks = []
...
def solve_all_wordles(words: WordList) -> None:
    ...
    """
    results: dict[WordleWord, int] = {}  # Stores the number of guesses it took to solve the Word
...
def interactive_prompt() -> None:
    ...
    while True:
        match (command := input("Enter a command ('h' for help, 'q' to quit): ")).lower().split():
            ...
            case ["autosolve", word]:
                solve_wordle(WordleWord(word), words, print_output = True)
